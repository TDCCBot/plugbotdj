...
async def getCount(ctx):
	try:
		...
		yield from bot.send_message(ctx.message.channel, "You have %s in your account." %str(int(dbOutput / 5 )* 10))#This is the calculation innit fam.
	except TypeError:
		yield from bot.send_message(ctx.message.channel, "You have 0 messages fam.")
	except Exception as e:
		yield from bot.send_message(ctx.message.channel, str(e))
...
async def on_message(message):
	...
	if message.content.startswith("$$help"): #Change depending on prefix obviously famalam
		...
		yield from bot.process_commands(message) # Run commands


#Final Initialisation
token = "" #Enter your bot token here, or use a different method. 
print("Starting uwu!")
bot.run(token)